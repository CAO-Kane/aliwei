...
>
> 读完本手册你应该能:
> - 画出 4 个 agent 共享的 `START → call_model → [run_tool | END]` 拓扑
> - 独立完成"在已有 agent 上加一个新 tool"的 5 步流程
> - 完整讲出 `ask_user` 暂停 → 用户回答 → 续流 的全链路

---

## 1. 为什么是 LangGraph

四个 agent(黑话翻译器 / 周报助手 / OKR 助手 / 复盘助手)必须共享一个执行地基,而不是每个工具单独维护 prompt + 工具调用 + 流式输出。原因:

- **工具必须后端执行。** `frontendTools` 没法和后端 `interrupt()` 协调,而且不能把只能服务端跑的副作用工具暴露给前端。
- **状态必须可暂停/续传。** `ask_user` 这种 HITL 工具要求图在"等用户回答"时把 state 序列化、resume 时从同一状态接着跑。`SqliteSaver` 帮我们做到这一点;前端工具做不到。
...
| toolId  | 状态扩展                  | 额外工具                                | graph 文件                  |
| ------- | ------------------------- | --------------------------------------- | --------------------------- |
| jargon  | `BaseState`               | (无)                                    | `jargon/graph.ts`           |
| weekly  | `BaseState`               | `collect_weekly_items`                  | `weekly/graph.ts`           |
| okr     | `OkrState` (+`okrDraft`)  | `breakdown_okr`, `search_past_okrs`     | `okr/graph.ts`              |
| review  | `ReviewState` (+`references`) | `search_past_reviews`               | `review/graph.ts`           |

**对照表说明:** 4 个 agent 共用 `base/` 下的工厂函数,只在 graph 文件里传 3 个差异点(状态扩展、工具列表、system prompt 来源)。
...
│   ├── model.ts                       # getChatModel (ChatOpenAI → Moark)
...
├── jargon/graph.ts                    # createJargonGraph
├── weekly/
│   ├── graph.ts                       # createWeeklyGraph
│   └── tools.ts                       # collectWeeklyItemsTool
├── okr/
│   ├── graph.ts                       # createOkrGraph
│   └── tools.ts                       # breakdownOkrTool + searchPastOkrsTool
└── review/
    ├── graph.ts                       # createReviewGraph
    └── tools.ts                       # searchPastReviewsTool
```

**`packages/domain` 的角色** —— 只剩两层:类型(`types.ts`)、TOOLS 注册表(`tools.ts`,纯元数据)。**所有 system prompt 都已经在 api 侧,不在 domain 里;阿里黑话词库也不再放在 domain,而是迁到 `packages/db/data/jargon.csv`,启动时由 db 包同步到 SQLite 的 `jargon` 表,`buildSystemPrompt` 通过 `@aliwei/db` 的 `getAllJargon()` + `formatJargonForPrompt()` 读。**

### 调用入口

- `services/chat-service.ts::streamChat()` — 根据 `toolId` 选图,持久化 user 消息,调 graph
- `routes/chat.ts` 的 `POST /chat` — 接 assistant-ui 的新消息
- `routes/chat.ts` 的 `POST /chat/continue` — 接 `ask_user` 的 resume,内部用 `Command({ resume })` 续流
...
**Per-tool 扩展**用 spread `spec`(不是 spread `State`):
...
自动把每次 state 更新序列化到存储。aliwei 用 `SqliteSaver`(`base/checkpointer.ts`):
...
```
final system prompt = buildSystemPrompt(TOOL_PROMPT)
                     = 通用规则 + 阿里黑话词库 + 工具职责(TOOL_PROMPT) + 通用规则结尾
```

`buildSystemPrompt` 在 `shared/prompts/base.ts`,**所有 agent 共享同一份包装逻辑**。每个 agent 只负责写自己的 `*_TOOL_PROMPT`(也就是"我作为这个工具的职责是什么")。
...
所以**写新 agent 的 TOOL_PROMPT 时不要重复"记得用工具"这种话术**。
...
- `threadId` / `toolId` / `system` — 这三个不指定 reducer,默认是 replace
...
`createBaseGraph(opts)` 是唯一被 4 个 agent 调用的入口。
...
const modelWithTools = opts.model.bindTools(allTools);  // ← base/graph.ts:27: bindTools 必须在 addNode 之前
...
在 `apps/api/src/agents/okr/tools.ts` 末尾加:
...
在 `apps/api/src/agents/okr/graph.ts` 的 `createBaseGraph` 调用的 `extraTools` 数组里加上:

```ts
import { breakdownOkrTool, searchPastOkrsTool, suggestOkrExamplesTool } from "./tools";

export function createOkrGraph(model) {
  return createBaseGraph({
    toolId: "okr",
    stateAnnotation: OkrState,
    systemPromptFn: () => buildSystemPrompt(OKR_TOOL_PROMPT),
    extraTools: [breakdownOkrTool, searchPastOkrsTool, suggestOkrExamplesTool],  // ← 加这里
    model,
  });
}
```

**不要改 `base/graph.ts`** — 那是共享地基,改它等于改所有 agent。

### Step 3:更新 TOOL_PROMPT

在 `apps/api/src/agents/shared/prompts/okr.ts` 的 `OKR_TOOL_PROMPT` 字符串里加一段描述,告诉模型**何时**调这个工具:
...
如果你新建了 prompt 文件,记得在 `shared/prompts/index.ts` 加导出:
...
(加 tool 不需要新建 prompt 文件时,跳过这步。)

### Step 5:验证

```bash
# 单元测试
pnpm --filter api test -- okr/tools
...
- **不在 `packages/domain` 里改任何文件** — prompts 已迁走,domain 是纯类型/数据层
- **不在 system prompt 里硬编码"用户可能的回答"** — 模型会僵化,失去泛化能力
- **不在 tool 体内 await 任何超过 5 秒的副作用** — 阻塞图节点;真要长任务请走异步节点
- **不要给 ask_user 之外的工具加 `interrupt()`** — 那会破坏图的同步性,改用返回结果让模型决策

---

## 6. HITL 完整流程(`ask_user`)
...
```
 ...
 7. 前端 AskUserCard 渲染选项
                ↓
 8. 用户点选 → POST /api/chat/continue { threadId, answer }
                ↓
 9. 后端 detectAskUserResume (chat-service.ts:46) 找到"待恢复的 ask_user"
                ↓
10. streamGraphToUIMessageStream(graph, new Command({ resume: answer }), { isResume: true })
                ↓
11. ★ isResume=true 时跳过所有 tool 事件
...
`chat-service.ts:46` 的核心问题:**怎么区分"用户刚回答了一个 ask_user"和"用户开始了一轮新对话"?**
...
只有"ask_user 是最新 assistant message 真正的最后一部分"时才走 resume,否则按新消息处理。
...
之前这套机制在 domain 层用 `InstructionsInjector` 组件动态注入。迁到 api 后改成在 `buildSystemPrompt` 的通用规则里**硬编码**这条提示。

**理由** ——

- 模型能不能"主动想到用 ask_user"是 prompt 问题,不是代码问题。改 tool 描述只能让模型"知道有这个工具",改 system prompt 才能让模型"知道何时调它"。
- 通用规则集中在 `base.ts` 一处,4 个 agent 的 TOOL_PROMPT 都不用重复写。
...
| 症状                              | 大概率原因                                              | 看哪里 |
| --------------------------------- | ------------------------------------------------------- | ------ |
| 前端一直循环发 POST `/chat`        | `detectAskUserResume` 误判,`isResume` 走错分支          | `chat-service.ts:46` 的 consumed 判定 |
...
集成测试用 langgraph 的 `FakeChatModel` 硬编码 AIMessage 序列,跑完整图验证 state 演化。`ask_user` 流程单独有单测覆盖 `detectAskUserResume` 的 "已消费"判定。`pnpm --filter api test` 跑全部。

---

## 9. 相关文档

- **本目录迁移设计:** `docs/superpowers/specs/2026-06-17-langgraph-migration-design.md`
